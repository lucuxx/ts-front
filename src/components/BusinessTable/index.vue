      ...
      <template slot-scope="scope">
        ...
        <template v-else-if="item.device">
          <div v-if="scope.row[item.prop] && scope.row[item.prop].length">
            <div
              v-for="(i, ind) in scope.row[item.prop]"
              :key="ind"
              class="event"
              @click="toDev(i.deviceId)"
            >
              {{ i.name }}
            </div>
          </div>
        ...
        <template
          v-else-if="
            scope.row.triggerType === '手动' && item.label === '触发主体'
          "
        >
          {{ scope.row[item.prop] || "-" }}
        </template>
        <template v-else>
          <span
            ...
            :class="{ event: item.event, weight: item.bold }"
            @click="detail(scope.row, item.event)"
          >
            {{ scope.row[item.prop] }}
