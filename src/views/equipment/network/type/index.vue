<template>
  <div class="app-container">
    <el-form
      ...
      v-show="showSearch"
    >
      <el-form-item label="" prop="networkName">
        <el-input
    ...
    <el-table v-if="refreshTable" v-loading="loading" :data="TypeList">
      ...
      <el-table-column prop="networkName" label="网络组件名称">
      ...
      <el-table-column prop="type" label="组件类型" :formatter="typeFormatter">
      </el-table-column>
      <el-table-column prop="clusterConf" label="集群方式">
        <template #default="{ row }">
          {{ row.clusterConf == 0 ? "共享配置" : "独立配置" }}
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    async getList() {
      try {
        this.loading = true;
        const { code, data } = await listType(this.queryParams);
        if (code === 200) {
    ...
    async handlePublish(row, val) {
      try {
        ...
        const res = await this.$confirm(
          `是否确认${name}名称为(${row.networkName})的数据项？`,
          "提示",
          {
        ...
        if (res === "confirm") {
          if (val == 1) {
            const { code, msg } = await enableType(row.id);
            this.resultFun(code, msg);
          } else {
            const { code, msg } = await disableType(row.id);
            this.resultFun(code, msg);
          }
        }
      } catch (err) {
    ...
    async handleDelete(row) {
      try {
        const res = await this.$confirm(
          '是否确认删除名称为"' + row.networkName + '"的数据项？',
          "提示",
          {
            confirmButtonText: "确定",
            cancelButtonText: "取消",
        ...
        if (res === "confirm") {
          const { code } = await delType(row.id);
          if (code === 200) {
            this.getList();
    ...
    typeFormatter(row) {
      if (!row.type) return "--";
      const currentType = this.dict.type.network_type.filter((item) => {
